...
- Name: gameplay.spec.ts >> Screen Adaptation >> canvas fills entire screen in landscape mode
- Location: e2e/gameplay.spec.ts:117:3
...
```ts
  27  | test.describe('Game Rendering', () => {
  28  |   test('canvas renders without errors', async ({ page }) => {
  ...
  35  | 
  36  |     await page.goto('/');
  37  |     await page.waitForSelector('canvas', { timeout: 10000 });
  38  |     await page.waitForTimeout(2000);
  39  | 
  40  |     // Dismiss tutorial
  41  |     const startButton = page.getByText('开始游戏');
  42  |     if (await startButton.isVisible().catch(() => false)) {
  ...
  48  |   });
  49  | 
  50  |   test('canvas is visible at different viewport sizes', async ({ page }) => {
  51  |     await page.goto('/');
  52  |     await page.waitForSelector('canvas', { timeout: 10000 });
  53  | 
  54  |     // Dismiss tutorial
  55  |     const startButton = page.getByText('开始游戏');
  56  |     if (await startButton.isVisible().catch(() => false)) {
  57  |       await startButton.click();
  58  |     }
  59  | 
  60  |     const canvas = page.locator('canvas');
  61  | 
  62  |     // Desktop
  63  |     await page.setViewportSize({ width: 1920, height: 1080 });
