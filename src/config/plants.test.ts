...
describe('植物配置', () => {
  it('4种植物: 豌豆射手(100阳光), 向日葵(50阳光), 坚果(50阳光), 樱桃炸弹(150阳光)', () => {
    expect(PLANT_CONFIGS).toHaveLength(4);
    expect(PLANT_CONFIG_MAP.get('peashooter')?.cost).toBe(100);
  ...
  it('向日葵: 每5秒产出25±10阳光', () => {
    ...
    expect(s?.produceInterval).toBe(5000);
    expect(s?.produceAmount).toEqual({ base: 25, variance: 10 });
  });
